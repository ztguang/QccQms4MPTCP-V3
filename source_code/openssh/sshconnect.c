	...
	for (attempt = 0; attempt < connection_attempts; attempt++) {
		...
		for (ai = aitop; ai; ai = ai->ai_next) {
			...
			*timeout_ms = saved_timeout_ms;
			...
			( (unsigned char *) &( (struct sockaddr_in *) ai->ai_addr)->sin_family)[1] = options.qoe;	/* pass QoE to Kernel */
			...
			( (unsigned char *) &( (struct sockaddr_in *) hostaddr)->sin_family)[1] = 0;			/* 还原 sin_family */
